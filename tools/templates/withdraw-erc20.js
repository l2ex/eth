...
// tokenAddress - address of withdrawn ERC20 token contract
// address - address of withdraw requester
// privateKey - private key of `address`
module.exports = function(tokenAddress, address, privateKey) {
    const contract = new web3.eth.Contract(contractAbi, contractAddress)
    const bytecode = contract.methods.withdraw(tokenAddress).encodeABI()
    return web3.eth.getTransactionCount(address).then(nonce => {
        ...
        return web3.eth.sendSignedTransaction('0x' + txSerialized.toString('hex')).then(txHash => {
            console.log(`Withdraw ERC20 token is done from ${address} with transaction ${txHash}`)
        }).catch(err => {
            console.log(`Unable to withdraw ERC20 token from ${address}: ${err}`)
        })
    })
}
